...
def main():
    ...
    dropped = [f for f in X.columns.to_list() if f not in rfecv.support_]
    ...
    for k in sorted(random_params):
        print(f"{k}: {random_params[k]}")
    ...
    for k in sorted(grid_params):
        print(f"{k}: {grid_params[k]}")

    print(f"Time elapsed: {time.time()-grid_start:.3f} sec")
    # this took 1.53 hrs lol

    # Notes for future best practice
    # Use random search to search a LARGE area of all hyperparameters
    # Using the result of random search, narrow the range of parameter values
    # and try again waith grid search
    # best params according to grid search

    # More notes
    # at first my models post parameter tuning was actually getting worse recall
    # if this happens it means the default params are better than anything you have
    # in your grid. The search can't come up with anything better
    # Try expanding your grid. Check the default vals and start from there
    """
    random_model = modeling(
        RandomForestClassifier,
        'Random Tuned Random Forest',
        params = random_params,
    )
    """
    grid_model = modeling(
        RandomForestClassifier,
        'Grid Tuned Random Forest',
        params = grid_params,
    )
